...
"use strict";
/* WEBPACK VAR INJECTION */(function(module) {// -*- coding: utf-8 -*-
/* !
    region header
    Copyright Torben Sickert (info["~at~"]torben.website) 16.12.2012
    ...
    endregion
*/// region imports
Object.defineProperty(exports,'__esModule',{value:true});var _keys=__webpack_require__(13);var _keys2=_interopRequireDefault(_keys);var _promise=__webpack_require__(14);var _promise2=_interopRequireDefault(_promise);var _asyncToGenerator2=__webpack_require__(6);var _asyncToGenerator3=_interopRequireDefault(_asyncToGenerator2);var _clientnode=__webpack_require__(0);var _clientnode2=_interopRequireDefault(_clientnode);var _keypress=__webpack_require__(15);var _keypress2=_interopRequireDefault(_keypress);var _configurator=__webpack_require__(4);var _configurator2=_interopRequireDefault(_configurator);var _pluginAPI=__webpack_require__(5);var _pluginAPI2=_interopRequireDefault(_pluginAPI);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}// NOTE: Only needed for debugging this file.
try{__webpack_require__(1)}catch(error){}// endregion
const handleError=(()=>{var _ref=(0,_asyncToGenerator3.default)(function*(plugins,configuration,error,services){try{yield _pluginAPI2.default.callStack('error',plugins,configuration,error,services)}catch(error){if(configuration.debug)throw error;else console.error(error)}});return function handleError(_x,_x2,_x3,_x4){return _ref.apply(this,arguments)}})();const main=(()=>{var _ref2=(0,_asyncToGenerator3.default)(function*(){// region load plugins
const{configuration,plugins}=yield _pluginAPI2.default.loadAll(_clientnode2.default.copyLimitedRecursively(_configurator2.default,-1,true));yield _pluginAPI2.default.callStack('initialize',plugins,configuration);if(plugins.length)console.info('Loaded plugins: "'+plugins.map(function(plugin){return plugin.internalName}).join('", "')+'".');for(const type of['pre','post'])yield _pluginAPI2.default.callStack(`${type}ConfigurationLoaded`,plugins,configuration,configuration,plugins.filter(function(plugin){return Boolean(plugin.configurationFilePath)}));// endregion
let services={};let servicePromises={};let exitTriggered=false;try{// region start services
services=yield _pluginAPI2.default.callStack('preLoadService',plugins,configuration,services);for(const name in services)if(services.hasOwnProperty(name))console.info(`Service "${name}" initialized.`);for(const plugin of plugins)if(plugin.api){services=yield _pluginAPI2.default.callStack(`preLoad${_clientnode2.default.stringCapitalize(plugin.internalName)}`+'Service',plugins,configuration,services);let result;try{// IgnoreTypeCheck
result=yield plugin.api.call(_pluginAPI2.default,'loadService',servicePromises,services,configuration,plugins)}catch(error){if(!(typeof error==='object'&&error!==null&&'message'in error&&error.message.startsWith('NotImplemented:')))throw new Error(`Plugin "${plugin.internalName}" `+(plugin.internalName===plugin.name?'':`(${plugin.name}) `)+`throws: ${_clientnode2.default.representObject(error)} `+'during asynchrone hook "loadService".')}if(result&&result.hasOwnProperty('name')&&typeof result.name==='string')if(result.hasOwnProperty('promise')&&typeof result.promise==='object'&&result.promise!==null&&'then'in result.promise){console.info(`Service "${result.name}" started.`);servicePromises[result.name]=result.promise}else console.info(`Service "${result.name}" loaded.`);services=yield _pluginAPI2.default.callStack(`postLoad${_clientnode2.default.stringCapitalize(plugin.internalName)}`+'Service',plugins,configuration,services,servicePromises)}servicePromises=yield _pluginAPI2.default.callStack('postLoadService',plugins,configuration,servicePromises,services);// endregion
// region register close handler
let finished=false;const closeHandler=function(){if(!finished)try{_pluginAPI2.default.callStackSynchronous('exit',plugins.slice().reverse(),configuration,services)}catch(error){throw error}finished=true};for(const closeEventName of _clientnode2.default.closeEventNames)process.on(closeEventName,closeHandler);// NOTE: Make "process.stdin" begin emitting events for any key press.
(0,_keypress2.default)(process.stdin);let cancelTriggered=false;process.stdin.on('keypress',(()=>{var _ref3=(0,_asyncToGenerator3.default)(function*(char,key){if(key&&key.name==='c'&&key.ctrl){if(cancelTriggered)console.warn('Stopping ungracefully.');else{cancelTriggered=true;console.log('You have requested to shut down all services. A '+'second request will force to stop ungracefully.');yield _pluginAPI2.default.callStack('shouldExit',plugins,configuration,services)}process.exit()}});return function(_x5,_x6){return _ref3.apply(this,arguments)}})());if('setRawMode'in process.stdin)// IgnoreTypeCheck
process.stdin.setRawMode(true);// endregion
try{yield _promise2.default.all((0,_keys2.default)(servicePromises).map(function(name){return servicePromises[name]}))}catch(error){}exitTriggered=true;yield _pluginAPI2.default.callStack('shouldExit',plugins,configuration,services);process.exit()}catch(error){handleError(plugins,configuration,error,services);if(!exitTriggered)try{yield _pluginAPI2.default.callStack('shouldExit',plugins,configuration,services)}catch(error){handleError(plugins,configuration,error,services)}if(configuration.debug)throw error;else console.error(error)}});return function main(){return _ref2.apply(this,arguments)}})();/* eslint-disable camelcase */// IgnoreTypeCheck
if(__webpack_require__.c[__webpack_require__.s]===module||typeof __webpack_require__!=='undefined')/* eslint-enable camelcase */main();exports.default=main;// region vim modline
// vim: set tabstop=4 shiftwidth=4 expandtab:
// vim: foldmethod=marker foldmarker=region,endregion:
// endregion
...
module.exports = require("keypress");
