...
### BEGIN INIT INFO
# Provides:          leaseRadWebAPPStagingServiceHandler
# Required-Start:
# Required-Stop:
...
### END INIT INFO
__NAME__='leaseRadWebAPPStagingServiceHandler'
# endregion
# Provides the main module scope.
function leaseRadWebAPPStagingServiceHandler() {
    # region constants
    local NEED_CURRENT_LOCATION=true
    local USER_NAME='http www-data'
    local PROCESS_ID_FILE_PATH='/tmp/webNodeStagingProcessID'
    local PROCESS_NAME_FILE_PATH='/tmp/webNodeStagingProcessName'
    local PRE_START_SCRIPT='docker rm webnodestaging &>/dev/null; docker-compose --file remoteStaging.yml pull; pushd ../ && mkdir --parents "log/reportedClientError" && touch log/proxyServer.txt; popd'
    local PROGRAM_START_FILE_PATH='docker-compose'
    local PROGRAM_START_ARGUMENTS='--project-name proxy --file remoteStaging.yml up --force-recreate'
    local PROGRAM_STOP_FILE_PATH="$PROGRAM_START_FILE_PATH"
    local PROGRAM_STOP_ARGUMENTS='--project-name proxy --file remoteStaging.yml stop'
    local PROGRAM_RELOAD_FILE_PATH="$PROGRAM_START_FILE_PATH"
    local PROGRAM_RELOAD_ARGUMENTS='--project-name proxy --file reloadStaging.yml up --force-recreate'
    local OUTPUT_FILE_PATH='../log/server.txt'
    # endregion
    # region controller
    ...
    local user_name
    local user_ensured=false
    for user_name in ${USER_NAME[*]}; do
        if [[ "$user_name" == "$USER" ]]; then
            user_ensured=true
        elif grep "^${user_name}:" /etc/passwd &>/dev/null; then
            echo "Use user \"$user_name\"." && \
            sudo -u "$user_name" "$0" "$@"
            return $?
        fi
    done
    if ! $user_ensured; then
