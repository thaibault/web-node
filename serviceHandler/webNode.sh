...
### BEGIN INIT INFO
# Provides:          leaseRadWebAPPServiceHandler
# Required-Start:
# Required-Stop:
...
### END INIT INFO
__NAME__='leaseRadWebAPPServiceHandler'
# endregion
...
function webNode() {
    ...
    local PROCESS_ID_FILE_PATH='/tmp/webNodeProcessID'
    local PROCESS_NAME_FILE_PATH='/tmp/webNodeProcessName'
    local PRE_START_SCRIPT='docker rm webnode &>/dev/null; docker-compose --file remote.yml pull; pushd ../ && mkdir --parents "log/reportedClientError" && touch log/proxyServer.txt; popd'
    local PROGRAM_START_FILE_PATH='docker-compose'
    local PROGRAM_START_ARGUMENTS='--project-name proxy --file remote.yml up --force-recreate'
    local PROGRAM_STOP_FILE_PATH="$PROGRAM_START_FILE_PATH"
    local PROGRAM_STOP_ARGUMENTS='--project-name proxy --file remote.yml stop'
    local PROGRAM_RELOAD_FILE_PATH="$PROGRAM_START_FILE_PATH"
    local PROGRAM_RELOAD_ARGUMENTS='--project-name proxy --file reload.yml up --force-recreate'
